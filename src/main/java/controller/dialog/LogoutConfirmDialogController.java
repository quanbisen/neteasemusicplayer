...
public class LogoutConfirmDialogController {
    ...
    public void onClickedConfirm(ActionEvent actionEvent) throws IOException {
        ...
        //清除登录存储文件
        if (leftController.getLOGIN_CONFIG_FILE().exists()) {  //如果文件存在，删除它
            User user = UserUtils.parseUser(leftController.getLOGIN_CONFIG_FILE());  //解析用户对象
            ...
            leftController.getLOGIN_CONFIG_FILE().delete();  //删除播放器的登录配置文件
        }
        ...
        leftController.getLabUserImage().setGraphic(imageView);
